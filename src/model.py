...
class PET:
    ...
    def get_logits(self, batch, full_vocab=False, logits_key='pet_logits'):
        # Get predicted labels
        full_logits = batch[logits_key]
        masked_logits = full_logits[batch['pet_flags'] == -1]
        if not full_vocab:
            masked_logits = masked_logits[:, self.label_ids]
        preds = masked_logits[:, self.label_ids].argmax(-1)
        pred_logits = masked_logits[:, self.label_ids]
        return nn.Softmax(dim=1)(masked_logits).detach().cpu(), batch['label_ids'].detach().cpu(), preds.detach().cpu(), pred_logits.detach().cpu()
...
class MLM:
    ...
    def prepare_input(self, batch):
        ...
        conditions = (pet_labels == rand_labels).view(-1, 1).repeat(1, sequence_length)
...
class MPET:
    ...
    def prepare_input(self, batch):
        ...
        mask_pos.masked_fill_(flags != 0, 0)  # Ignore unmaskable
        ids.masked_fill_(mask_pos, self.tokenizer.mask_token_id)
        batch['mpet_input_ids'] = ids
        batch['mpet_labels'] = ids[mask_pos == 1]
        batch['mpet_mask_pos'] = mask_pos

        
    def forward_step(self, batch, logits_key='mpet_logits'):
        # Perform MLM forward and store output back
        batch[logits_key] = self.model(input_ids=batch['mpet_input_ids'],
                                       attention_mask=batch['attention_mask'],
                                       token_type_ids=batch['token_type_ids'])[0]

    def get_loss(self, batch, full_vocab=False, logits_key='mpet_logits'):
    ...
    def get_logits(self, batch, full_vocab=False, logits_key='mpet_logits'):

        # Get predicted labels
        full_logits = batch[logits_key]

        masked_logits = full_logits[batch['mpet_mask_pos'] == 1]

        # Use softmax here to involve full vocabulary
        masked_logits = nn.Softmax(dim=1)(masked_logits)
        # Select target token probability
        masked_logits = masked_logits.gather(1, batch['mpet_labels'].view(-1, 1)).squeeze()
        masked_values = torch.zeros(batch['mpet_mask_pos'].size(), dtype=torch.float, device=masked_logits.device)
        masked_values[batch['mpet_mask_pos'] == 1] = masked_logits
        masked_values, _ = masked_values.max(dim=1)
        
        pred_logits = full_logits[batch['pet_flags'] == -1]
        preds = pred_logits[:, self.label_ids].argmax(-1)
        if not full_vocab:
            pred_logits = pred_logits[:, self.label_ids]
        # return nn.Softmax(dim=1)(pred_logits).detach().cpu(), batch['label_ids'].detach().cpu()
        return nn.Softmax(dim=1)(pred_logits).detach().cpu(), masked_values.detach().cpu(), preds.detach().cpu(), pred_logits[:, self.label_ids].detach().cpu()

    def get_confidence(self, batch, logits_key='mpet_logits'):

        full_logits = batch[logits_key]
        masked_logits = full_logits[batch['pet_flags'] == -1]
        masked_logits = nn.Softmax(dim=1)(masked_logits[:, self.label_ids])
        return torch.gather(masked_logits, 1, batch['label_ids'].unsqueeze(1)).detach().cpu()
...
def get_pet_mappers(tokenizer, reader, model, device, pet_method, mask_rate):
    ...
    mpet = MPET(tokenizer, reader, model, mask_rate) if mask_rate > 0.0 else None
    if pet_method == 'pet':
        return PET(tokenizer, reader, model, device), mlm, mpet
    if pet_method == 'diffpet':
        return DiffPET(tokenizer, reader, model, device), mlm, mpet
