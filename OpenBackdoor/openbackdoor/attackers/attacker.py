...
class Attacker(object):
    ...
    def __init__(
        ...
        # self.poison_file_path = os.path.join('./models/', 'mix-' + poisoner['name'] + '-' + poisoner['poison_rate'], 'poison.csv')
    ...
    # def save_poison_data(self, path, victim: Victim, dataset: List):
        
    #     data = pd.DataFrame(self.poison(victim, dataset, 'eval')['test-poison'])
    #     data.to_csv(path, index=False, sep='\t')
    def save_poison_data(self, path, victim: Victim, dataset: List, format: str = 'csv'):
        ''' csv data follows "label,text"
            tsv data follows "text\tlabel"
        '''
        data = pd.DataFrame(self.poison(victim, dataset, 'eval')['test-poison'])
        if format == 'csv':  
            data = data.iloc[:,[1, 0]]
        data.to_csv(path, index=False, sep='\t' if format == 'tsv' else ',', header=False)
