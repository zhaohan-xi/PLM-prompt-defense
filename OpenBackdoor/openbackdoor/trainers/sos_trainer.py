    ...
    def sos_train(self, model, dataset, metrics):
        ...
        for epoch in range(self.sos_epochs):
            ...
            for batch in self.dataloader["train"]:
                ...
                nan_rows = torch.any(output.isnan(),dim=1)
                loss = self.loss_function(output[~nan_rows], batch_labels[~nan_rows])
                loss.backward()
            ...
            print('total_loss', total_loss, 'len(self.dataloader["train"])', len(self.dataloader["train"]))
