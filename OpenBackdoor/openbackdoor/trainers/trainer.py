...
class Trainer(object):
    ...
    def train_one_epoch(self, epoch: int, epoch_iterator):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # adaptive attack -- random masking
            if self.adaptive:
                _loss = []
                for _ in range(3): 
                    mask = torch.rand(batch_inputs['input_ids'].shape, device="cuda") > 0.3
                    mask_batch_inputs = {'input_ids': batch_inputs['input_ids'] * mask,
                                        'attention_mask': batch_inputs['attention_mask'] * mask}
                    mask_batch_inputs['input_ids'][mask_batch_inputs['input_ids']==0] = 1
                    mask_batch_inputs['input_ids'][:, 0] = 0
                    
                    _output = self.model(mask_batch_inputs)
                    _logits = _output.logits
                    _loss.append(mseloss(_logits, logits))
                loss += 1 * torch.mean(torch.stack(_loss))
